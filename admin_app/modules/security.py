...
Features:
  ...
  - Manual Employee ID entry fallback
  ...
  - Uses filter= keyword for Firestore to suppress UserWarning
...
class SecurityModule:
    ...
    def _build_ui(self):
        ...
        tk.Label(top, text=f"Guard: {self.current_user.get('display_name','')}  [{role}]",
    ...
    def _build_scanner_panel(self, parent):
        tk.Label(parent, text=f"📅  Today: {_today()}",
                 bg="#1a2740", fg="#f0c040",
        ...
        self.manual_status = tk.Label(parent, text="",
                                      bg="#0d1b2a", fg="#27ae60", font=("Arial",9))
    ...
    def _build_log_panel(self, parent):
        ...
        self.log_tree = ttk.Treeview(parent, columns=cols, show="headings", height=28)
        for col, lbl, w in [
            ("time",   "Time",    90),("emp_id","Emp ID",  100),
            ("name",   "Name",   160),("action","IN/OUT",   80),("method","Method",100),
        ]:
    ...
    def _stop_camera(self):
        ...
        self.cam_canvas.create_text(170, 128,
            text="📷  Camera feed will appear here", fill="#555", font=("Arial",11))
    ...
    def _camera_loop(self):
        ...
        last_scanned = None; last_scan_time = 0
        while self.scanning and self.cap.isOpened():
            ...
            for obj in decoded:
                ...
                if emp_id != last_scanned or (now_ts - last_scan_time) > 3:
                    last_scanned = emp_id; last_scan_time = now_ts
    ...
    def _process_scan(self, emp_id: str, method: str = "CAMERA"):
        ...
        if not name:
            try:
                ...
                self.manual_status.config(text=f"Error: {ex}", fg="#e74c3c"); return
        ...
        try:
            ...
            self._update_session(emp_id, _today(), action, time_s)
        except Exception as ex:
            self._save_local_log(emp_id, name, action, _today(), time_s, method)
            self.manual_status.config(
        ...
        self.last_action_lbl.config(text=f"{emoji}  Marked {action}", fg=colour)
        self.last_time_lbl.config(text=f"Time: {time_s}")
        self.manual_status.config(text=f"✔ {emp_id} — {name} — {action} at {time_s}", fg=colour)
        self.manual_var.set("")
    ...
    def _determine_action(self, emp_id: str) -> str:
        tod = _today()
        try:
            # FIX: use filter= keyword to suppress Firestore UserWarning
            from google.cloud.firestore_v1.base_query import FieldFilter
            logs = self.db.collection("attendance_logs") \
                .where(filter=FieldFilter("employee_id", "==", emp_id)) \
                .where(filter=FieldFilter("timestamp", ">=", tod+"T00:00:00")) \
    ...
    def _update_session(self, emp_id, date_str, action, time_str):
        doc_id   = f"{emp_id}_{date_str}"
        ref      = self.db.collection("sessions").document(doc_id)
        existing = ref.get()
        if existing.exists:
            updates = {"updated_at": datetime.now().isoformat()}
            if action=="IN":  updates["in_time"]  = time_str
            else:
                ...
                if in_t and in_t!="—":
                    try:
                        ...
                        diff = (datetime.strptime(time_str,fmt) -
                                datetime.strptime(in_t,fmt)).seconds/3600
                        updates["duty_status"] = "absent" if diff<4 else ("half" if diff<7 else "full")
            ...
            ref.set({
                ...
                "out_time":    "—", "duty_status": "absent",
                "ot_status":   "none", "duty_hours": 0,
                "manual":      False, "created_at": datetime.now().isoformat(),
            })

    def _manual_mark(self, force: str = None):
        emp_id = self.manual_var.get().strip().upper()
        if not emp_id:
            self.manual_status.config(text="❌ Enter Employee ID first.", fg="#e74c3c"); return
        if force:
            name = self._emp_cache.get(emp_id,"")
            if not name:
                try:
                    doc = self.db.collection("employees").document(emp_id).get()
                    if doc.exists: name = doc.to_dict().get("name","")
                    else:
                        self.manual_status.config(text=f"❌ '{emp_id}' not found.",fg="#e74c3c"); return
                except Exception as ex:
                    self.manual_status.config(text=str(ex),fg="#e74c3c"); return
            ts = datetime.now().isoformat()
            try:
                self.db.collection("attendance_logs").add({
                    "employee_id": emp_id, "timestamp": ts,
                    "action": force, "session": 1,
            ...
            self.manual_status.config(text=f"✔ {emp_id} — Force {force} at {_now_str()}", fg=colour)
            self.log_tree.insert("", 0,
        ...
        try:
            ...
            for doc in docs:
                ...
                self.log_tree.insert("", "end",
                    ...
                    tags=("in" if action=="IN" else "out",))
                count += 1
            self.log_count_lbl.config(text=f"Today's scans: {count}  |  {tod}")
