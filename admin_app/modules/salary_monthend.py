...
#   3. Results table — Emp ID, Name, Department, Mode, Days, Gross, Advance, Net
#   4. Export to Excel:
#      - Sheet 1: "All Employees" — full salary summary
#      - Sheet 2: "Cash Payments"  — only CASH mode, sorted by amount (hand-payment list)
#      - Sheet 3: "Bank/UPI"       — non-cash employees
#   5. Save salary slips to Firestore under "salary" collection
#   6. Auto-clear advance after salary saved
...
from modules.salary import calculate_salary
...
class MonthEndPanel(tk.Frame):
    """Full Month-End salary generation panel — plug into any tab."""

    def __init__(self, parent, role="admin"):
        super().__init__(parent, bg="#0d1b2a")
        self.role    = role
        self.results = []   # list of salary dicts from last generation
        self._build_ui()
    ...
    def _build_ui(self):
        ...
        hdr.pack(fill="x")
        tk.Label(hdr, text="📅 Month-End Salary Generation",
        ...
        tk.Button(ctrl, text="📊 Export Excel",
        ...
        cols = ("no","emp_id","name","dept","mode","present","absent",
                "base","ot_pay","bonus","advance","gross","net","status")
    ...
    def _confirm_generate(self):
        ...
        if not messagebox.askyesno(
                ...
                f"• Advance will be deducted & cleared\n"
    ...
    def _generate_all(self):
        ...
        for idx, emp in enumerate(employees):
            ...
            try:
                # Load sessions for this employee — filter by month in Python
    ...
    def _load_existing(self):
        """Load already-generated slips from Firestore for selected month."""
        month_idx = MONTHS.index(self.month_var.get()) + 1
        try:
            year = int(self.year_var.get().strip())
        except ValueError:
            return
        month_str = f"{year}_{month_idx:02d}"
    ...
    def _populate_table(self, mode_filter="All"):
        ...
        self.summary_lbl.config(
            text=f"  📊 Total Employees: {shown}   "
                 f"💰 Total Gross: ₹{total_gross:,.0f}   "
                 f"✅ Total Net: ₹{total_net:,.0f}   "
                 f"│   💵 Cash Payments: {cash_count} employees  ₹{total_cash:,.0f}   "
                 f"🏦 Bank/UPI: {bank_count} employees  ₹{total_bank:,.0f}"
        )
    ...
    def _export_excel(self):
        ...
        style_title(ws1, 1, 1, f"HYPE HR MANAGEMENT — Salary Report  {month_name} {year}", 14)
        ws1.cell(row=2, column=1, value=f"Generated: {datetime.now().strftime('%d-%b-%Y %I:%M %p')}")
        ...
        headers1 = ["#","Emp ID","Name","Department","Pay Mode",
                    ...
                    "OT Pay","Bonus","Advance","Gross Pay","Net Pay","Status"]
        ...
        for i, r in enumerate(self.results, start=1):
            ...
            text_cell(ws1, row, 3, r.get("name",""),        align=Alignment(horizontal="left",vertical="center"))
            text_cell(ws1, row, 4, r.get("department",""),  align=Alignment(horizontal="left",vertical="center"))
            ...
            money_cell(ws1, row, 13, gross)
            money_cell(ws1, row, 14, net)
            text_cell(ws1, row, 15, r.get("_status",""))
        ...
        ws1.merge_cells(start_row=tr, start_column=1, end_row=tr, end_column=12)
        c = ws1.cell(row=tr, column=1, value="GRAND TOTAL")
        c.font = TOTAL_FONT; c.fill = TOTAL_FILL; c.alignment = RIGHT; c.border = BORDER
        money_cell(ws1, tr, 13, grand_gross).font = TOTAL_FONT
        money_cell(ws1, tr, 14, grand_net).font   = TOTAL_FONT
        for col in range(1, 16): ws1.cell(row=tr, column=col).fill = TOTAL_FILL

        set_col_widths(ws1, [4,11,22,16,14,8,8,13,13,11,11,11,13,13,10])

        # ══════════════════════════════════════════════════════════════════════
        # SHEET 2 — Cash Payment List (hand-payment register)
        # ══════════════════════════════════════════════════════════════════════
        ws2 = wb.create_sheet("Cash Payments")
        style_title(ws2, 1, 1, f"💵 CASH PAYMENT LIST — {month_name} {year}", 9)
        ws2.cell(row=2, column=1,
                 value="Hand this list to the accountant/cashier for cash disbursement")
        ws2.cell(row=2, column=1).font = Font(italic=True, size=9, color="555555")
        ...
        for i, r in enumerate(cash_emps, start=1):
            ...
            text_cell(ws2, row, 3, r.get("name",""),       align=Alignment(horizontal="left",vertical="center"))
            text_cell(ws2, row, 4, r.get("department",""), align=Alignment(horizontal="left",vertical="center"))
            text_cell(ws2, row, 5, r.get("designation",""),align=Alignment(horizontal="left",vertical="center"))
            money_cell(ws2, row, 6, net)
            text_cell(ws2, row, 7, "")  # Signature blank
            text_cell(ws2, row, 8, "")  # Date blank
            text_cell(ws2, row, 9, "")  # Remarks blank
        ...
        c = ws2.cell(row=tr2, column=1, value=f"TOTAL CASH TO ARRANGE: ₹{cash_total:,.2f}")
        c.font = Font(bold=True, size=12, color="CC0000"); c.fill = TOTAL_FILL
        ...
        style_title(ws3, 1, 1, f"🏦 BANK / UPI TRANSFER LIST — {month_name} {year}", 8)
        ...
        messagebox.showinfo(
            "✅ Export Complete",
            f"Excel saved:\n{path}\n\n"
            f"📄 Sheet 1 — All Employees ({len(self.results)} records)\n"
            f"💵 Sheet 2 — Cash Payments ({len(cash_emps)} employees  ₹{cash_total:,.0f})\n"
            f"🏦 Sheet 3 — Bank/UPI ({len(bank_emps)} employees  ₹{bank_total:,.0f})",
            parent=self
        )
        # Auto-open the file
        try:
